...
class Ship:	
	...
	def goto(self, point, override=False):
		if self.way:
			if override:
				...
				self.way = self.way + [(self.way[-1][1], point)]
		else:
			self.way = [(self.position, point)]
	
	def addCanon(self, canon):
		if self.config.canons:
			self.config.canons += [canon]
		else:
			self.config.canons = [canon]
	
	def shootCanon(self, index=0):
		if index < len(self.config.canons):
			return self.config.canons[index].shoot()
		return None
	...
	def tick(self, clk):
		...
		for canon in self.config.canons:
			# propagate tick
			canon.tick(clk)
